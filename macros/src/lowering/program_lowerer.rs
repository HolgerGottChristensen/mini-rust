...
use chalk_integration::lowering::lower_adt_size_align;
use chalk_integration::{Identifier, RawId};
use chalk_ir::{AdtId, AssocTypeId, BoundVar, ClosureId, DebruijnIndex, FnDefId, ForeignDefId, GeneratorId, ImplId, OpaqueTyId, TraitId, TyVariableKind, Variance};
use chalk_ir::cast::Cast;
use chalk_solve::rust_ir;
use chalk_solve::rust_ir::{AssociatedTyValueId, GeneratorDatum, GeneratorInputOutputDatum, GeneratorWitnessDatum, GeneratorWitnessExistential, OpaqueTyDatum, OpaqueTyDatumBound};
use crate::lowering::env::{AdtIds, AdtKinds, AutoTraits, Env, FnDefIds, FnDefKinds, GeneratorIds, GeneratorKinds, OpaqueTyIds, OpaqueTyVariableKinds, TraitIds, TraitKinds};
use crate::lowering::{LowerParameterMap, LowerResult, LowerTypeKind, LowerWithEnv};
...
impl ProgramLowerer {
    ...
    pub fn extract_ids(&mut self, program: &MiniFile, raw_ids: &[RawId]) -> LowerResult<()> {
        for (item, &raw_id) in program.items.iter().zip(raw_ids) {
            match item {
                MiniItem::Enum(_) => todo!(),
                MiniItem::Struct(s) => {
                    let type_kind = s.lower_type_kind()?;
                    let id = AdtId(raw_id);
    ...
    pub fn lower(self, program: &MiniFile, raw_ids: &[RawId]) -> LowerResult<Program> {
        ...
        for (item, &raw_id) in program.items.iter().zip(raw_ids) {
            ...
            match item {
                ...
                MiniItem::Impl(_) => todo!(),
                MiniItem::Struct(s) => {
                    let identifier = Identifier::from(s.ident.to_string());
                    let adt_id = AdtId(raw_id);
                    adt_data.insert(adt_id, Arc::new((s, adt_id).lower(&empty_env)?));
                    //adt_reprs.insert(adt_id, Arc::new(d.repr.lower(&empty_env)?)); todo: impl
                    //adt_size_aligns.insert(adt_id, Arc::new(lower_adt_size_align(&d.flags))); todo: impl
                    let n_params = s.all_parameters().len();

                    // Todo: Figure out what the heck variances are
                    let variances = (0..n_params)
